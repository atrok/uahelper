...
class CouchDB {
  ...
  check_views(response, create_view) {
    ...
    return new Promise((resolve, reject) => {
      ...
      logger.log(database_name + ": Start working with DB");
      ...
      db.get('_all_docs', {
        ...
        logger.log(database_name + ': Check exisiting views in _all_docs, processing results:');
        ...
        try {
          res.rows.forEach(function (row, key) {
            logger.log(database_name + ': Design doc id: ' + row.id + ', rev: ' + row.value.rev);
            if (row.id === '_design/test' || row.id === '_design/test2') {
              ...
              views_def.view_definitions.forEach(function (value) {
                if (views.find(x => x == value.name)) {
                  ...
                  logger.log(database_name + ": '" + value.name + "' function, adjusted mask:" + parseInt(mask).toString(2));
                }
              });
          ...
          var p = undefined;
          if (mask != FULL_MASK) {
            logger.log(database_name + ': Masks aren\'t equal: ' + parseInt(mask).toString(2) + " vs. " + parseInt(FULL_MASK).toString(2));
  ...
  create_view(logger, options) {
    ...
    return new Promise(resolve => {
      ...
      views_def.view_definitions.forEach(function (value) {
        ...
        if (mask & value.flag) {
          logger.log(database_name + ": View already exists: " + value.name);
        } else {
          logger.log(database_name + ": Missing view " + value.name);
        }
        Object.assign(func, value.function);

      });

      logger.log(database_name + ": Creating new view");
      ...
      db.save("_design/test", rev, func, function (err, res) {
        ...
        resolve(database_name + ": Created succesfully, id:" + res.id + ", rev: " + res.rev + ", view:" + view); // returning Promise after views are created;
      });
